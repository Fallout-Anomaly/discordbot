...
module.exports = new ApplicationCommand({
    ...
    run: async (client, interaction) => {
        ...
        // Use Promise wrapper for cleaner async/await
        ...
        if (row === null) return interaction.editReply({ content: '❌ Database error.' });

        if (!row) {
            // Atomic insert
            await new Promise(resolve => db.run('INSERT OR IGNORE INTO users (id) VALUES (?)', [target.id], () => resolve()));
            row = {
                id: target.id,
        ...
        if (isSelf && stats.points > 0) {
            ...
            collector.on('collect', async i => {
                ...
                db.run(
                    `UPDATE users SET ${statColumn} = ${statColumn} + 1, stat_points = stat_points - 1 WHERE id = ? AND stat_points > 0`,
                    [i.user.id],
                    function (err) {
                        if (err) return i.editReply({ content: '❌ Database error.' });
                        if (this.changes === 0) return i.editReply({ content: '❌ Not enough points!' });
