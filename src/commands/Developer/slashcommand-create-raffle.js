...
module.exports = new ApplicationCommand({
    ...
    run: async (client, interaction) => {
        ...
        if (action === 'enter') {
            ...
            if (donorTier !== 'none') {
                const savings = raffle.entry_cost - entryFee;
                discountInfo = `\n💎 ${tierInfo.badge} Donator discount: -${savings} caps!`;
            }

            const embed = new EmbedBuilder()
                .setTitle('✅ Entered Raffle!')
                .setDescription(`${raffle.title}\n${prizeDisplay}`)
                .addFields(
                    { name: '👤 Entered', value: `<@${targetUser.id}>`, inline: true },
                    { name: '🎟️ Your Entries', value: userEntryCount.toString(), inline: true },
                    { name: '💰 Cost Paid', value: entryFee > 0 ? `${entryFee} caps` : 'Free', inline: true }
                )
                .setColor('#2ecc71')
                .setFooter({ text: 'More entries = Better odds to win!' });
            ...
            if (donorTier !== 'none') {
                embed.addFields(
                    { name: '💎 Donator Bonus', value: `${tierInfo.badge} ${(tierInfo.multiplier * 100).toFixed(0)}% better odds!`, inline: false }
                );
            }
